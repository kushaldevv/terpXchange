        ...
        if let user = firebaseAuth.getCurrentUser() {
            ...
            usersRef.updateData(["reviews": FieldValue.arrayUnion([
                ...
                if let error = error {
                    ...
                    self.reviews.append(Review(id: UUID().uuidString, rating: Int(rating), details: details, timestamp: Date(), reviewerUID: user.uid, reviewerName: user.displayName ?? "Unknown"))
